...
class AppCheckConfiguration {
    ...
    static func configure() {
        ...
        AppLogger.auth.info("App Check: DISABLED for local emulator testing (DEBUG mode)")
        AppLogger.auth.debug("Note: App Check will be ENABLED in RELEASE builds on physical devices")
        AppLogger.auth.debug("To test App Check, build in RELEASE mode on a real iOS device")
        #else
        // Use App Attest for production (iOS 14+)
        // Provides device-level attestation of app authenticity
        let providerFactory = AppAttestProviderFactory()
        AppCheck.setAppCheckProviderFactory(providerFactory)
        AppLogger.auth.info("App Check: Using App Attest Provider for production")
        #endif
    }
}
