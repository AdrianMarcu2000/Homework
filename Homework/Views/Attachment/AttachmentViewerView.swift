...
/// Unified attachment viewer that replaces AttachmentDetailView and AttachmentContentView
/// Supports all material types: images, PDFs, ODT files, links, YouTube videos, and forms
struct AttachmentViewerView: View {
    ...
    private func driveFileViewer(_ driveFile: DriveFile) -> some View {
        ...
        if ["jpg", "jpeg", "png", "gif", "heic", "heif", "bmp"].contains(fileExtension) {
            ...
            // ODT viewer
            if let fileData = fileData {
                ODTViewerView(odtData: fileData, fileName: driveFile.title)
            } else {
                VStack {
                    Spacer()
                    Text("Loading ODT...")
